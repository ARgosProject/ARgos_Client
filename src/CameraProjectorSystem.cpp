...
namespace argosClient {
  ...
  bool CameraProjectorSystem::isValid() const {
    return camera.isValid() && projector.isValid()
      && rotCamToProj.rows != 0 && rotCamToProj.cols != 0 && transCamToProj.rows !=0 && transCamToProj.cols != 0;
  }

  vector<cv::Point2f> CameraProjectorSystem::getProjected(const vector<cv::Point3f>& objectPoints,
                                                          const cv::Mat& rotObjToCam, const cv::Mat& transObjToCam){
