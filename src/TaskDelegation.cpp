...
int TaskDelegation::connect(const std::string& ip, const std::string& port) {
  try {
    ...
    Logger::Log::info("Intentando conectar al servidor " + ip + ":" + port + "...");
    boost::asio::connect(*_tcpSocket, _tcpResolver->resolve({ip, port}));
    Logger::Log::success("Conectado satisfactoriamente.");
    _ip = ip; _port = port;

    return 0;
  }
  catch(boost::system::system_error const& e) {
    Logger::Log::error("No se pudo conectar al servidor de ARgos. " + std::string(e.what()));
    _error = true;
  }
...
int TaskDelegation::reconnect() {
  try {
    ...
    Logger::Log::info("Intentando reconectar al servidor de ARgos...");
    boost::asio::connect(*_tcpSocket, _tcpResolver->resolve({_ip, _port}));
    Logger::Log::success("Reconectado satisfactoriamente.");

    return 0;
  }
  catch(boost::system::system_error const& e) {
    Logger::Log::error("No se pudo reconectar al servidor de ARgos. " + std::string(e.what()));
    _error = true;
  }
...
int TaskDelegation::send() {
  try {
    ...
    Logger::Log::error("Se perdió la conexión al servidor de ARgos al enviar. " + std::string(e.what()));
    _buff.clear();
    _error = true;
  }
...
int TaskDelegation::receive(paper_t& paper) {
  ...
  try {
    ...
    Logger::Log::error("Se perdió la conexión al servidor de ARgos al recibir. " + std::string(e.what()));
    _tcpSocket->close();
    //mat = cv::Scalar::all(0);
    _error = true;
  }
...
void TaskDelegation::proccessPaper(StreamType& st, paper_t& paper) {
  Logger::Log::success("Nuevo Paper recibido. Size: " + std::to_string(st.size));
